...
public class SurpriseBox {
	...
	/**
	 * Effect types 
	 * 1 - "confused mode", make panda roll to opposite direction than it is poked 
	...
	public SurpriseBox(int x, int y, World world) {
		...
		hitBox = new Rectangle(x, y, Art.box.getRegionWidth(),
				Art.box.getRegionHeight());
		type = MyGame.random.nextInt(8) + 1;

		// Creating body definition
		BodyDef bodyDef = new BodyDef();
		bodyDef.type = BodyType.DynamicBody;
		bodyDef.position.set(new Vector2((x + hitBox.width / 2)
				* MyGame.WORLD_TO_BOX, (y + hitBox.height / 2)
				* MyGame.WORLD_TO_BOX));
		...
		shape.setAsBox(hitBox.width / 2 * MyGame.WORLD_TO_BOX, hitBox.height
	...
	public void regenerate(int x, int y) {
		...
		body.setTransform((x + hitBox.width / 2) * MyGame.WORLD_TO_BOX,
				(y + hitBox.height / 2) * MyGame.WORLD_TO_BOX, 0f);
		regenerate = false;
	}
}
