...
public class Bee {
	...
	private final Vector2 pandasPosition;
	...
	public Bee(World world, Vector2 pandasPosition) {
		...
		hitBox = new Rectangle(0, 0, Art.bee[0].getRegionWidth(),
				Art.bee[0].getRegionHeight());
		...
		shape.setAsBox(hitBox.width / 7 * MyGame.WORLD_TO_BOX, hitBox.width
	...
	public void draw(SpriteBatch spriteBatch) {
		spriteBatch.draw(Art.bee[frame], body.getPosition().x
	...
	public void update(float deltaTime) {
		...
		case WAITING:
			...
			if (timer < 0) {
				state = ATTACKING;
				moveAngle = (float) Math.atan2(pandasPosition.y
						* MyGame.BOX_TO_WORLD - hitBox.y, pandasPosition.x
						* MyGame.BOX_TO_WORLD - hitBox.x);
				float newVelocityX = (float) Math.cos(moveAngle) * moveSpeed
						* 2 * MyGame.WORLD_TO_BOX;
				float newVelocityY = (float) Math.sin(moveAngle) * moveSpeed
						* 2 * MyGame.WORLD_TO_BOX;
	...
	public void regenerate() {
		...
		targetX = MyGame.random.nextInt(MyGame.SCREEN_WIDTH
				- (int) hitBox.width);
		targetY = MyGame.random.nextInt(MyGame.SCREEN_HEIGHT
				- (int) hitBox.height);
