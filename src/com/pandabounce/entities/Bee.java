...
public class Bee {
	...
	public Bee( World world, Vector2 pandasPosition){
		...
		bodyDef.position.set(new Vector2((0+  hitBox.width/2)*MyGame.WORLD_TO_BOX , (0+ hitBox.height/2)*MyGame.WORLD_TO_BOX ));
		...
		shape.setAsBox(hitBox.width/7*MyGame.WORLD_TO_BOX, hitBox.width/2.8f*MyGame.WORLD_TO_BOX);
	...
	public void update(float deltaTime) {
		...
		hitBox.x = body.getPosition().x*MyGame.BOX_TO_WORLD;
		hitBox.y = body.getPosition().y * MyGame.BOX_TO_WORLD;
		...
		case FLYING_IN:
			...
			if(tempSignX != signX || tempSignY != signY){
				...
				timer = 3f + MyGame.random.nextFloat()*3;
			}
		...
		case WAITING:
			...
			if(timer < 0){
				state = ATTACKING;
				moveAngle = (float) Math.atan2(	pandasPosition.y*MyGame.BOX_TO_WORLD - hitBox.y , pandasPosition.x*MyGame.BOX_TO_WORLD - hitBox.x);
				float newVelocityX = (float)Math.cos(moveAngle)* moveSpeed * 2 * MyGame.WORLD_TO_BOX ;
				float newVelocityY = (float)Math.sin(moveAngle)* moveSpeed * 2 * MyGame.WORLD_TO_BOX ;
		...
		case ATTACKING:
			if(!MyGame.SCREEN_RECTANGLE.contains(hitBox)){
				state = IDLE;
				dontRegenerateFor = 2 + MyGame.random.nextInt(5);
			}
			break;
		}
	}
	...
	public void regenerate(){
		...
		targetX = MyGame.random.nextInt(MyGame.SCREEN_WIDTH-(int)hitBox.width);
		targetY = MyGame.random.nextInt(MyGame.SCREEN_HEIGHT-(int)hitBox.height);
		...
		float newVelocityX = (float)Math.cos(angle)* moveSpeed * MyGame.WORLD_TO_BOX ;
		float newVelocityY = (float)Math.sin(angle)* moveSpeed * MyGame.WORLD_TO_BOX ;
		body.setTransform(x * MyGame.WORLD_TO_BOX, y * MyGame.WORLD_TO_BOX, angle - (float)Math.PI/2);
		body.setLinearVelocity(newVelocityX, newVelocityY);
